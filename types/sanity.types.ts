...
export type SponsorshipPackage = {
  ...
  benefits?: InternationalizedArrayText;
...
export type Member = {
  ...
  major?: InternationalizedArrayString;
  studyYear?: "prep" | "1" | "2" | "3" | "4" | "msc" | "phd" | "alumni";
  image?: {
    asset?: SanityImageAssetReference;
    media?: unknown;
    hotspot?: SanityImageHotspot;
...
// Source: sanity/lib/queries.ts
// Variable: NAV_VEHICLES_QUERY
// Query: *[_type == "vehicle" && defined(slug.current)] | order(year desc, title asc) {    _id,    title,    "slug": slug.current,    year,    type,    "tagline": coalesce(tagline[language == $locale][0].value, tagline[language == $defaultLocale][0].value)  }
export type NAV_VEHICLES_QUERY_RESULT = Array<{
  _id: string;
  title: string | null;
  slug: string | null;
  year: number | null;
...
// Query: *[_type == "member" && active == true]    | order(isLead desc, order asc, name asc) {      _id,      name,      department,      isLead,      "role": coalesce(role[language == $locale][0].value, role[language == $defaultLocale][0].value),      "major": coalesce(major[language == $locale][0].value, major[language == $defaultLocale][0].value),      studyYear,      image{  ...,  "alt": coalesce(alt, ""),  asset->{    _id,    "lqip": metadata.lqip,    "dimensions": metadata.dimensions  }},      linkedin,      github,      email    }
export type MEMBERS_QUERY_RESULT = Array<{
  ...
  role: string | null;
  major: string | null;
  studyYear: "1" | "2" | "3" | "4" | "alumni" | "msc" | "phd" | "prep" | null;
  image: {
    asset: {
      _id: string;
      lqip: string | null;
...
export type SPONSORS_QUERY_RESULT = Array<{
  _id: string;
  name: string | null;
  tier: "bronze" | "gold" | "main" | "platinum" | "silver" | "supplier" | null;
  website: string | null;
  logo: {
    asset: {
      _id: string;
...
// Source: sanity/lib/queries.ts
// Variable: SPONSORSHIP_PACKAGES_QUERY
// Query: *[_type == "sponsorshipPackage"] {    _id,    tier,    featured,    "priceLabel": coalesce(priceLabel[language == $locale][0].value, priceLabel[language == $defaultLocale][0].value),    "benefits": coalesce(benefits[language == $locale][0].value, benefits[language == $defaultLocale][0].value),    "note": coalesce(note[language == $locale][0].value, note[language == $defaultLocale][0].value)  }
export type SPONSORSHIP_PACKAGES_QUERY_RESULT = Array<{
  _id: string;
  tier: "bronze" | "gold" | "main" | "platinum" | "silver" | "supplier" | null;
